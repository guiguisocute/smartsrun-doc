...
已收录的参数、后缀、来源与验证范围可在[学校预设](/guide/schools)中查询。
...
## Go 2.0 开发版的手动登出与热点切换

手动登出会暂停该账号的自动认证，其他受管理的有线账号继续独立运行。暂停在服务重启后保留；手动登录、重新登录或切回该校园账号成功后恢复。保存新的配置或重启路由器也会重新应用自动认证策略。状态命令会列出手动暂停的账号。

启用“登出时自动切换热点上网”后，当前校园账号先核对所选线路的在线身份，确认会话下线后，再连接已选热点并检查连通性。登出失败或结果不明时不会切网；热点失败时会分别说明校园会话已下线和热点失败原因，账号仍保持暂停。未配置热点时保留当前线路。对其他有线账号执行登出不会改动当前无线出口。

定时切到的热点会在静默结束后尝试返回校园网；用户主动选择的热点会保留。服务重启可以恢复有有效临时记录的定时返回，路由器重启后不凭当前热点连接猜测此前的切换归属。
