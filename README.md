...
下载页默认按路由器型号选择，`data/devices.json` 保存 OpenWrt 官方设备型号、硬件版本、固定固件目标及包架构映射。搜索支持品牌、型号和常用中文品牌名；官方固件可直接匹配，第三方固件或未收录型号使用本机查询结果校正，不从商品型号推断其包管理器。

```sh
node scripts/sync-devices.mjs
```

该脚本显式读取已固定的 OpenWrt 24.10.8 / 25.12.2 概览与各目标 `profiles.json`，记录来源 URL 与原始 SHA256，全部核对后原子替换快照；失败保留原文件。当前收录 1,928 个设备条目、172 组固件目标。型号被收录不等于本项目已有对应包或完成真机验收。浏览器和站点构建都无需访问官方设备服务。
